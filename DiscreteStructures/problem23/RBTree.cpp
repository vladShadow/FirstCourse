...
void RBTree::removeNode(int data) {
...
Node* RBTree::find(int value) {
    if (root == nullptr) throw invalid_argument("There's no such node in the tree");
    Node* curr = root;
    while (curr->data != value) {
        if (curr->data <= value) curr = curr->left;
        else curr = curr->right;
        if (curr == nullptr) throw invalid_argument("There's no such node in the tree");
    }
    return curr;
}
